		...
		switch (this.type) {
			...
			case MESSAGE_TYPE.CONNECT:
				...
				if (this.willMessage != undefined) {
					remLength +=
						UTF8Length(this.willMessage.destinationName) + 2;
	...
	ClientImpl.prototype.store = function (prefix, wireMessage) {
		...
		switch (wireMessage.type) {
			case MESSAGE_TYPE.PUBLISH:
				...
				storedMessage.payloadMessage.qos =
					wireMessage.payloadMessage.qos;
			...
			default:
				throw Error(
					format(ERROR.INVALID_STORED_DATA, [key, storedMessage])
				);
		}
		...
		try {
			switch (wireMessage.type) {
				case MESSAGE_TYPE.CONNACK:
					...
					var sequencedMessages = sequencedMessages.sort(function (
						a,
						b
					) {
					...
					for (
						var i = 0, len = sequencedMessages.length;
						i < len;
						i++
					) {
						...
						if (
							...
							var pubRelMessage = new WireMessage(
								MESSAGE_TYPE.PUBREL,
								{
									messageIdentifier:
										sentMessage.messageIdentifier,
				...
				case MESSAGE_TYPE.PUBACK:
					var sentMessage = this._sentMessages[
						wireMessage.messageIdentifier
					];
					// If this is a re flow of a PUBACK after we have restarted receivedMessage will not exist.
					if (sentMessage) {
						delete this._sentMessages[
							wireMessage.messageIdentifier
						];
						localStorage.removeItem(
							"Sent:" +
				...
				case MESSAGE_TYPE.DISCONNECT:
					...
					this._disconnected(
						...
						format(ERROR.INVALID_MQTT_MESSAGE_TYPE, [
							wireMessage.type,
						])
					);
					break;
	...
	var Client = function (host, port, path, clientId) {
		...
		if (arguments.length == 2) {
			...
			if (typeof port !== "number" || port < 0)
				throw new Error(
					format(ERROR.INVALID_TYPE, [typeof port, "port"])
				);
			if (typeof path !== "string")
				throw new Error(
					format(ERROR.INVALID_TYPE, [typeof path, "path"])
				);
		...
		if (typeof clientId !== "string" || clientIdLength > 65535)
			throw new Error(
				format(ERROR.INVALID_ARGUMENT, [clientId, "clientId"])
			);
		...
		this.connect = function (connectOptions) {
			...
			if (connectOptions.hosts) {
				...
				if (!usingURIs) {
					...
					if (
						connectOptions.hosts.length !=
						connectOptions.ports.length
					)
						throw new Error(
							format(ERROR.INVALID_ARGUMENT, [
								connectOptions.ports,
